...
type FilteredUser = Pick<User<ObjectId>, '_id' | 'email' | 'meta' | 'name' | 'roles'>;
function filterFields(cursor: Cursor<User<ObjectId>>) {
...
export async function getUsers(query?: { email?: string }) {
    ...
    return useCollection('Users', (Users) => filterFields(Users.find(_query)).toArray());
}

export async function getUser(userId: string) {
    const user = await useCollection('Users', (Users) => Users.findOne({ _id: new ObjectID(userId) }));
...
export async function generateInviteLink(role: Roles, inviter: ObjectId) {
    const { insertedCount, insertedId } = await useCollection('InviteLinks', (InviteLinks) =>
