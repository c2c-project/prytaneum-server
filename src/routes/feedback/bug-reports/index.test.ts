...
describe('/bug-reports', () => {
    ...
    describe('POST /:townhallId', () => {
        it('should have status 200', async () => {
            ...
            jwtSpy.mockResolvedValueOnce(user);
            const { status } = await request(app)
                .post(`${endpoint}/${new ObjectID().toHexString()}`)
                .type('form')
                .set('Cookie', [`jwt=${faker.random.alphaNumeric()}`])
                .send(bugReportForm);
            expect(status).toStrictEqual(200);
        });
        it('should have status 400 if creation of bug report fails', async () => {
            const collectionSpy = jest.spyOn(DB, 'useCollection');
            collectionSpy.mockResolvedValueOnce(user);
            collectionSpy.mockResolvedValueOnce({
                insertedCount: 0,
        ...
        it('should have status 400 for an incomplete form', async () => {
            const copy: Partial<BugReportForm> = { ...bugReportForm };
        ...
        it('should have status 400 since bug report data is not sent', async () => {
            const collectionSpy = jest.spyOn(DB, 'useCollection');
            collectionSpy.mockResolvedValueOnce(user);
            const jwtSpy = jest.spyOn(jwt, 'verify');
            jwtSpy.mockResolvedValueOnce(user);
        ...
        it('should have status 400 since bug report description is empty', async () => {
            const copy: Partial<BugReportForm> = { ...bugReportForm };
            copy.description = '';
            const collectionSpy = jest.spyOn(DB, 'useCollection');
            collectionSpy.mockResolvedValueOnce(user);
    ...
    describe('PUT /:reportId', () => {
        ...
        it('should have status 400 for an incomplete bug report form', async () => {
            const partial: Partial<BugReportForm> = {
                ...bugReportForm,
            };
    ...
    describe('DELETE /:reportId', () => {
        ...
        it('should have status 404 if the bug report is not found', async () => {
            const collectionSpy = jest.spyOn(DB, 'useCollection');
            collectionSpy.mockResolvedValueOnce(user);
    ...
    describe('PUT /:reportId/reply', () => {
        ...
        it('should have status 400 is it fails to submit reply to bug report', async () => {
            const collectionSpy = jest.spyOn(DB, 'useCollection');
            collectionSpy.mockResolvedValueOnce({ ...user, roles: ['admin'] });
            collectionSpy.mockResolvedValueOnce({ modifiedCount: 0 });
            const jwtSpy = jest.spyOn(jwt, 'verify');
